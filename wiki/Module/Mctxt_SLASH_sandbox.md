...
function p._dialogue( str, size, class, style )
	...
	local add_category = ''
	for i, l in ipairs(t) do
		-- handle modifiers and add trailing &r
		local availableMods = 'cr' -- put all available modifiers here
		local pttn, lineclass = '^$%[[' .. availableMods .. ']+%]', ''
		local modifiers = l:match(pttn)
		if modifiers then
			if modifiers:match('c') then lineclass = 'centertxt' end
			if modifiers:match('r') then lineclass = 'righttxt' end
			l = l:gsub(pttn, '')
		end
		l = l:gsub('^%s*$', '%%%%AMPERSAND%%%%nbsp;')
