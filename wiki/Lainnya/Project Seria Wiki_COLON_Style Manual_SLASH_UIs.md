...
*{{T|Crafting Table}} - A template that creates a Vanilla Minecraft crafting table. Although Project Seria uses a different UI than the vanilla one, the two are so similar in function that it does not cause any issues for readers.
...
*{{T|UI}} - A Generic template that creates a Project Seria Inventory UI. Highly configurable.
...
*{{Code|{{((}}Potion UI{{))}}}} - Creates a Potion brewing recipe UI. Not currently used.
...
**All css related to this may be found at [[MediaWiki:Common.css]].
